...
if (window.location.search === '?step=2') {
  ...
  const btnInstall = document.getElementById('hw-button-install-id');
  ...
  function validateFields() {
      if (!dbHost.value || !dbPort.value || !dbUsername.value || !dbName.value) {
          ...
          btnInstall.setAttribute('disabled', 'disabled'); // เพิ่ม attribute disabled
          ...
          btnInstall.removeAttribute('disabled'); // ลบ attribute disabled
      }
  }
...
if (window.location.search === '?step=2') {
  const btnInstallDatabase = document.getElementById('hw-button-install-id-database');
  btnInstallDatabase.addEventListener("click", () => {
    ...
    xhttp.send(`dbHost=${dbHost} &dbPort=${dbPort} &dbUsername=${dbUsername} &dbPassword=${dbPassword} &dbName=${dbName}`);
  });
}
